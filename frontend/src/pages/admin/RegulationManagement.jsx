  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-semibold text-[var(--text-primary)] tracking-tight">Academic Regulations</h1>
          <p className="text-sm text-[var(--text-muted)] mt-0.5">Manage institutional assessment patterns and passing rules</p>
        </div>
        <button onClick={() => { resetForm(); setIsModalOpen(true); }} className="flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-white rounded-xl transition-all" style={{ background: 'var(--primary-500)' }} onMouseEnter={e => e.currentTarget.style.background = 'var(--primary-600)'} onMouseLeave={e => e.currentTarget.style.background = 'var(--primary-500)'}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {regulations?.map((reg) => (
          <Card key={reg.id} className="relative overflow-hidden group border-[var(--border)] hover:border-[var(--primary-200)] transition-all duration-300">
            <div className="flex justify-between items-start mb-3">
              <div>
                <h3 className="text-base font-semibold text-[var(--text-primary)]">{reg.name}</h3>
                <span className={`inline-flex items-center px-2 py-0.5 rounded-lg text-[10px] font-semibold mt-1.5 ${
                  reg.is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-[var(--surface-tertiary)] text-[var(--text-muted)]'
                }`}>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-[40px] w-full max-w-2xl overflow-hidden shadow-2xl border border-slate-100 animate-in fade-in zoom-in duration-200">
            <div className="px-6 py-4 border-b border-[var(--border)] bg-[var(--surface-secondary)] flex items-center justify-between">
              <div>
                <h2 className="text-base font-semibold text-[var(--text-primary)]">{editingReg ? 'Update Regulation' : 'New Regulation'}</h2>
                <p className="text-[var(--text-muted)] text-xs mt-0.5">Assessment weightages &amp; passing rules</p>
              </div>
            ...
            <form onSubmit={(e) => { e.preventDefault(); saveMutation.mutate(formData); }} className="p-10 space-y-8">
              ...
              <div className="bg-[var(--surface-secondary)] p-6 rounded-xl border border-[var(--border)]">
                <h4 className="text-sm font-semibold text-[var(--text-primary)] mb-4 flex items-center gap-2">
                  <Settings2 size={16} className="text-[var(--primary-400)]" /> Academic Rule Engine
                </h4>
              ...
              <div className="flex items-center justify-between gap-4 pt-4">
                ...
                <button type="submit" disabled={saveMutation.isPending} className="text-white h-12 px-8 rounded-xl font-semibold text-sm transition-all flex items-center justify-center gap-2" style={{ background: 'var(--primary-500)' }} onMouseEnter={e => !saveMutation.isPending && (e.currentTarget.style.background = 'var(--primary-600)')} onMouseLeave={e => !saveMutation.isPending && (e.currentTarget.style.background = 'var(--primary-500)')}>
